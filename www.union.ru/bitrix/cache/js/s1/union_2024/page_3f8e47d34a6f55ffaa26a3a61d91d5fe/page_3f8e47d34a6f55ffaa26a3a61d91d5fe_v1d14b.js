...
/* End */
;
; /* Start:"a:4:{s:4:"full";s:105:"/local/templates/union_2024/components/bitrix/catalog.element/.default/card2.0/sctipts.js?175639439215182";s:6:"source";s:89:"/local/templates/union_2024/components/bitrix/catalog.element/.default/card2.0/sctipts.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
...
const setHeightVideoItem = () => {
    const nodes = document.querySelectorAll('.inrter_row .video-item')
    const nodeHeight = document.querySelector('.inrter_row .swiper-slide:not(.video-item)')
    if (nodes && nodeHeight) {
        nodes.forEach(node => {
            const iframe = node.querySelector('iframe')
            if (iframe) {
                iframe.style.height = nodeHeight.clientHeight + 'px'
            }
        })
    }
}
...
const swiperKorobLocNew = new Swiper('.korob-swiper-new', {
    slidesPerView: "1.5",
    spaceBetween: 45,
    freeMode: true,
    loop: false,
    //centeredSlides: true,
    navigation: {
        nextEl: '.swiper-button-next-custom-price-new',
        prevEl: '.swiper-button-prev-custom-price-new',
    },
    scrollbar: {
        el: '.swiper-scrollbar-swiper-price-new',
...
if (swiperKorobLocNew) {
    let node = document.querySelector('.price-line-new')
    if (node) {
        node.style.display = 'block'
    }
    swiperKorobLocNew.on('slideChange', function (swiperTmp) {
        //transform
        //console.log(swiperTmp);
        //swiper.destroy();
        //initSwiper()
    });
}

const swiperKorobLoc = new Swiper('.korob-swiper', {
    slidesPerView: "1.5",
    spaceBetween: 45,
    freeMode: true,
    loop: false,
    //centeredSlides: true,
    navigation: {
        nextEl: '.swiper-button-next-custom-korob',
        prevEl: '.swiper-button-prev-custom-korob',
    },
    scrollbar: {
        el: '.swiper-scrollbar-swiper-korob',
        // Makes the Scrollbar Draggable
        draggable: false,
        // Snaps slider position to slides when you release Scrollbar
        snapOnRelease: true,
...
if (swiperKorobLoc) {
    let node = document.querySelector('.korob-swiper')
    if (node) {
        node.style.display = 'block'
    }
    swiperKorobLoc.on('slideChange', function (swiperTmp) {
        //transform
        //console.log(swiperTmp);
        //swiper.destroy();
        //initSwiper()
...
const nodeMainSlider = document.getElementById('product-slider-wrap2')
if (nodeMainSlider) {
    new Swiper(nodeMainSlider, {
        slidesPerView: "1",
        spaceBetween: 0,
        freeMode: false,
        loop: false,
        ...
        on: {
            afterInit: (swiper) => {
                ...
                calculatePaginationPosition(swiper)
            },
            resize: (swiper) => {
                calculatePaginationPosition(swiper)
            },
...
const swiperLoc = new Swiper('.inrter-swiper', {
    slidesPerView: "1.5",
    spaceBetween: 45,
    freeMode: true,
    loop: false,
    //centeredSlides: true,
    navigation: {
        nextEl: '.swiper-button-next-custom-inter',
        prevEl: '.swiper-button-prev-custom-inter',
    },
    scrollbar: {
        el: '.swiper-scrollbar-swiper-inter',
        // Makes the Scrollbar Draggable
        draggable: false,
        // Snaps slider position to slides when you release Scrollbar
        snapOnRelease: true,
    ...
    on: {
        afterInit: (swiper) => {
            setHeightVideoItem();
            setTimeout(setHeightVideoItem, 100);
            setTimeout(setHeightVideoItem, 110);
            setTimeout(setHeightVideoItem, 120);
            setTimeout(setHeightVideoItem, 130);
            setTimeout(setHeightVideoItem, 140);
            setTimeout(setHeightVideoItem, 150);
            setTimeout(setHeightVideoItem, 1000);
            initArrowSliderSwiper(swiper)
        },
    },
    breakpoints: {
...
if (swiperLoc) {
    let node = document.querySelector('.inrter_row')
    if (node) {
        node.style.display = 'block'
    }
    swiperLoc.on('slideChange', function (swiperTmp) {
        //transform
        //console.log(swiperTmp);
        //swiper.destroy();
        //initSwiper()
...
$(document).ready(function () {
    let timerId = setInterval(() => {
        ...
        if (node) {
            ...
            const swiperVariants = new Swiper('.swiperVariants', {
                slidesPerView: "1.5",
                spaceBetween: 45,
                freeMode: true,
                navigation: {
                    nextEl: '.swiper-button-next-custom-swiperVariants',
                    prevEl: '.swiper-button-prev-custom-swiperVariants',
                },
                scrollbar: {
                    el: '.swiper-scrollbar-swiperVariants',
                    // Makes the Scrollbar Draggable
                    draggable: false,
                    // Snaps slider position to slides when you release Scrollbar
                    snapOnRelease: true,
            ...
            if (swiperVariants) {
                let node = document.querySelector('.variants_row')
                if (node) {
                    node.style.display = 'block'
                }
            }

            const swiperStyle = new Swiper('.swiperStyles', {
                slidesPerView: '1.5',
                spaceBetween: 45,
                freeMode: true,
                navigation: {
                    nextEl: '.swiper-button-next-custom-styles',
                    prevEl: '.swiper-button-prev-custom-styles',
                },
                scrollbar: {
            ...
            if (swiperStyle) {
                let node = document.querySelector('.styles_row')
                if (node) {
                    node.style.display = 'block'
                }
            }
...
$("[data-fancybox-korob]").click((event) => {
    event.preventDefault()
    const target = event.currentTarget
    const groupName = target?.getAttribute('data-fancybox-korob')
    if (groupName) {
        $.fancybox.open(document.querySelectorAll("[data-fancybox-korob="+groupName+"]"), {
            margin: 0, // делаю окно во весь экран без отступов
            //clickContent    : false, // отключаю второе увеличение при уже открытой картинке
            buttons: [ //оставляю только кнопку закрытия
                "close"
            ...
            beforeLoad: function( instance ) {
                const nodeUl = instance.Thumbs.$list
                if (nodeUl) {
                    nodeUl.addClass("custom-ul-fancybox");
                }
            },
        });
    }
...
// раскрывает/скрывает скрытые отделки по кнопке
$(function(){
    $('.options-hidden--toggle').on('click', function(e) {
        let $list = $(this).toggleClass('active').closest('.toggle-block');
        if ($list) {
...
$('.under-price-slider').on('init', function(event, slick) {
    initArrowSlider(slick)
    calculatePaginationPosition(slick, 'slick')
});
$('.under-price-slider').on('setPosition', function(event, slick) {
    calculatePaginationPosition(slick, 'slick')
});
$('.under-price-slider').on('swipe', function(event, slick) {
    calculatePaginationPosition(slick, 'slick')
});
$('.under-price-slider').on('reInit', function(event, slick) {
    calculatePaginationPosition(slick, 'slick')
});
$('.under-price-slider').slick({
    //lazyLoad: 'ondemand',
    dots: true,
    arrows: true,
    infinite: false,
...
$('#under-price-slider [data-fancybox]').fancybox({
    margin: 0, // делаю окно во весь экран без отступов
    //clickContent    : false, // отключаю второе увеличение при уже открытой картинке
    buttons: [ //оставляю только кнопку закрытия
        "close"
...
/* End */
;
; /* Start:"a:4:{s:4:"full";s:114:"/local/templates/union_2024/components/bitrix/catalog.element/.default/shareblock/viewed_scripts.js?17505063592235";s:6:"source";s:99:"/local/templates/union_2024/components/bitrix/catalog.element/.default/shareblock/viewed_scripts.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
$(document).ready(function () {
    let timerId = setInterval(() => {
        let node = typeof Swiper !== "undefined"
        if (node) {
...
; /* /local/templates/union_2024/components/bitrix/catalog.element/.default/card2.0/sctipts.js?175639439215182*/
; /* /local/templates/union_2024/components/bitrix/catalog.element/.default/shareblock/viewed_scripts.js?17505063592235*/

//# sourceMappingURL=page_3f8e47d34a6f55ffaa26a3a61d91d5fe.map.js